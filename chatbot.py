...
# Import necessary libraries from both scripts
import requests

# --- Common API setup (if applicable, otherwise define within each bot's section) ---
API_URL = "https://router.huggingface.co/v1/chat/completions"
headers = {
    "Authorization": "Bearer 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", # Secure this with st.secrets!
}
...
if chatbot_choice == "Janus 7B Chatbot":
    st.set_page_config(page_title="🤖 Janus 7B Chatbot", page_icon="🧠", layout="centered")
    st.markdown(
        """
...
# --- Hugging Face Chatbot Section ---
    ...
    st.set_page_config(page_title="🤖 HuggingFace Chatbot", page_icon="🤖", layout="centered") # Note: page_config can only be set once per run
    st.markdown(
        """
    ...
    def display_hf_chat():
        for i, chat in enumerate(st.session_state.hf_history):
            if chat['role'] == 'user':
                st.markdown(f"**You:** {chat['content']}")
            else:
